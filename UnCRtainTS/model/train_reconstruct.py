...
def iterate(model, data_loader, config, writer, mode="train", epoch=None, device=None, results_path=None):
        ...
        if mode != "train": # val or test
            with torch.no_grad():
                ...
                for bdx in range(batch_size):
                    ...
                    file_name = batch['target']['S2 path'][bdx].split('/')[-1].replace('_s2', '')
                    results_list.append({'file_name': file_name, 'cloud_cover': batch['input']['coverage'][bdx], **extended_metrics})
                    relative_path = '/'.join(batch['target']['S2 path'][bdx].split('/')[-3:])
                    s2_src_path = batch['target']['S2 path'][bdx]
                    s2_tar_path = os.path.join(results_path, relative_path)
                    pred = out[bdx].cpu().numpy()[0, :, :, :]
                    # Copy .tif image to output folder
                    os.makedirs(os.path.dirname(s2_tar_path), exist_ok=True)
                    shutil.copy(s2_src_path, s2_tar_path)
                    src = rasterio.open(s2_tar_path, 'r+', driver='GTiff')
                    # Clip the predicted cloudfree data to [0, 1]
                    pred = np.clip(pred, 0, 1)
                    # Scale by 10000 (as done in the original dataset)
                    pred = pred * 10000
                    src.write(pred.astype(rasterio.uint16))
                    src.close()
