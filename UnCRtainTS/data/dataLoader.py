...
class SEN12MSCR(Dataset):
    ...
    def get_paths(self):  # assuming for the same ROI+num, the patch numbers are the same
        ...
        paths = []
        rois_S1 = self.splits[self.split]
        for roi in tqdm(rois_S1):
            ...
            for pdx, _ in enumerate(paths_S1):
                ...
                if broken_file in paths_S2[pdx]:
                    continue
        ...
        print(f'Found {len(paths)} patches for {self.split} split of region {self.region}\n')
        return paths
